...
1. В `.env` укажите:
   ...
   - `TROJAN_USER_NAME` / `TROJAN_USER_PASSWORD` — логин и пароль первого пользователя. Trojan аутентифицирует только по паролю (поле `password` — массив строк в конфиге), поэтому `TROJAN_USER_NAME` используется исключительно для удобства администрирования и нигде в самом trojan не участвует. Если пароль не задан, при первом запуске он будет сгенерирован автоматически и выведен в лог контейнера (`docker compose logs trojan`).
